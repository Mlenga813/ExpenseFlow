...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const { action, comment, signature } = body; // action: APPROVE, REJECT, INFO_REQUESTED
    ...
    // Check if the specific user is assigned (for non-admin roles)
    if (!['SUPER_ADMIN', 'ADMIN'].includes(user.role)) {
      const assignedField = `assigned${getAssignField(request.currentStep)}Id` as keyof typeof request;
      if (request[assignedField] && request[assignedField] !== user.id) {
    ...
    const requestorMsg = action === 'APPROVE'
      ? `Your request ${request.requestNumber} has been approved by ${user.name} and moved to the next step`
      : action === 'REJECT'
        ? `Your request ${request.requestNumber} has been rejected by ${user.name}`
        : `${user.name} has requested additional information for ${request.requestNumber}`;

    await db.notification.create({
      data: {
        userId: request.requestedById,
        title: action === 'APPROVE' ? 'Request Approved' : action === 'REJECT' ? 'Request Rejected' : 'Info Requested',
    ...
    // Notify next reviewer if approved
    if (action === 'APPROVE') {
        ...
        if (nextReviewer) {
          await db.notification.create({
            data: {
              ...
              message: `${request.requestNumber}: ${request.title} needs your review`,
              type: 'SUBMISSION',
              link: 'approvals',
            },
          });
...
function getAssignField(step: number): string {
  switch (step) {
    case 1: return 'OpsManager';
    case 2: return 'ChiefAccountant';
    case 3: return 'GeneralManager';
